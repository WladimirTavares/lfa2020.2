...
\item Para toda linguagem $L$, $DROPOUT(L,c) = \{w | w \in L \mbox{ com todos os caracteres $c$ deletados}\}$. Por exemplo,
se L = \{abc, abbc, abcc\} então $DROPOUT(L,b) = \{ac, acc\}$. Prove que para todo $a$, se L é regular então $DROPOUT(L,a)$ é regular.
Dica: Utilizando uma representação formal de L (autômato determinístico ou expressão regular), construa uma representação formal de $DROPOUT(L,a)$

\item Para toda linguagem $L$, $prefix(L) = \{ x | xy \in L \mbox{ para algum y} \in \Sigma^*\}$. Por exemplo, se $L = \{ab,bab\}$ então 
$prefix(L) = \{a,ab,b,ba,bab\}$. 

\begin{enumerate}
\item Construa um DFA que aceita a linguagem $L$ definida pelo seguinte expressão regular $a^*b^*cc^*$.

Exemplo de palavras aceitas: c, ac, bc, abc. 

Exemplo de palavras não aceitas: aacb, bac, abca.

\item Construa um $\varepsilon$-NFA a partir do DFA da questão anterior que aceita a linguagem $prefix(L)$.

\item Mostre que se $L$ é regular então $prefix(L)$ é regular.
